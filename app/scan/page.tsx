...
function StaffBlock({ row }: { row: Row }) {
  ...
  const SECTION_META = {
    safe:  { label: "No Allergens Detected",  subtitle: "None of your allergens found — confirm with staff before ordering", mark: "✓", bg: "#f0fdf4", border: "#bbf7d0", textColor: "#15803d" },
    ask:   { label: "Check First",            subtitle: "May contain your allergens — ask staff before ordering", mark: "?", bg: "#fff7db", border: "#f4dd8d", textColor: "#854d0e" },
    avoid: { label: "Avoid",                  subtitle: "Contains your allergens — do not order without staff confirmation", mark: "!", bg: "#fff1f0", border: "#f3c5c0", textColor: "#b91c1c" },
  };
  ...
  // Safe Order Card — one-tap shareable summary for server or family
  function SafeOrderCard({ restaurantName, safe, ask, avoid, allergens }: {
    restaurantName: string | null;
    safe: Row[];
    ask: Row[];
    avoid: Array<Row | AvoidRow>;
    allergens: string[];
  }) {
    ...
    function buildCard() {
      ...
      const safeItems = safe.slice(0, 5).map((r) => r.item.split("—")[0].trim()).join(", ");
      const askItems = ask.slice(0, 3).map((r) => r.item.split("—")[0].trim()).join(", ");
      const avoidItems = avoid.slice(0, 3).map((r) => r.item.split("—")[0].trim()).join(", ");

      const allAskQuestions = ask.slice(0, 3).flatMap((r) => r.staffQuestions.slice(0, 2));
      const questions = allAskQuestions.length > 0
        ? "\n\nPlease confirm with your kitchen:\n" + allAskQuestions.map((q) => `• ${q}`).join("\n")
        : "";

      return [
        `I have a ${allergenList} allergy${restaurant ? ` — scanning the menu ${restaurant}` : ""}.`,
        safeItems   ? `✓ Looks safe: ${safeItems}` : null,
        askItems    ? `? Need to confirm: ${askItems}` : null,
        avoidItems  ? `✗ I will avoid: ${avoidItems}` : null,
        questions,
        "\nAlways verify with kitchen staff. Generated by AllergEats — allergeats.com",
      ].filter(Boolean).join("\n");
    }

    async function handleCopy() {
      await navigator.clipboard.writeText(buildCard()).catch(() => {});
      setCopied(true);
    ...
    return (
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
          ...
          <div>
            <div style={{ fontSize: 14, fontWeight: 800, color: "var(--c-text)" }}>Safe Order Card</div>
            <div style={{ fontSize: 11, color: "var(--c-sub)" }}>Copy to share with staff or family</div>
          </div>
        </div>
        <button
          onClick={handleCopy}
        ...
        >
          {copied ? "✓ Copied to clipboard" : "Copy Safe Order Card"}
        </button>
      </div>
    );
  }
            ...
            <>
              ...
              <div style={{ background: "var(--c-card)", border: "1px solid var(--c-border)", borderRadius: 20, padding: 20, boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
                ...
                <div style={{ display: "flex", gap: 16, marginTop: 10, flexWrap: "wrap" }}>
                  <SummaryPill count={results.safe.length}  label="No Allergens" color="#15803d" bg="#f0fdf4" />
              ...
              {/* Safety reminder — always visible, never dismissible */}
              <div style={{
                ...
                <div style={{ fontSize: 12, color: "var(--c-text)", lineHeight: 1.55 }}>
                  <strong>Always confirm with staff before ordering.</strong> Results are based on listed ingredients only — menus change and cross-contamination cannot be detected from text.
                  <span style={{ display: "block", marginTop: 3, color: "var(--c-sub)" }}>Detection via ingredient text analysis + USDA FoodData Central database.</span>
                </div>
              </div>
              ...
              {/* Safe Order Card — share with server or family */}
              {(results.avoid.length > 0 || results.ask.length > 0 || results.safe.length > 0) && (
                <SafeOrderCard
                  restaurantName={loadedRestaurant}
                  safe={results.safe}
                  ask={results.ask}
                  avoid={results.avoid}
